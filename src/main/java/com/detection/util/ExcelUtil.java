...
public class ExcelUtil{
     ...
     */
    public static <T> List<T> readExcelBean(InputStream is, Class<T> tClass,int startRow)
        ...
        T uBean = null;
        for (int i = startRow; i < list.size(); i++) {// i=1是因为第一行不要
            ...
            for (int j = 0; j < listStr.size(); j++) {
                ...
                if (datastring.length()>0&&datastring!=null) {
                    ...
                    if (type==String.class){
                        ...
                        field.set(uBean,BigDecimal.valueOf(Long.parseLong(datastring)));
                    }
                }
            }
            listBean.add(uBean);
